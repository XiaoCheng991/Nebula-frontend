...
const GlobalHeader: React.FC<GlobalHeaderProps> = ({
    ...
    // 文字颜色根据滚动状态变化
    const navTextClasses = isScrolled
        ? 'text-slate-700 dark:text-slate-300'
        : 'text-slate-900 dark:text-white';

    return (
        <header className={`w-full sticky top-0 z-50 transition-all duration-300 ${className}`} aria-label="站点头部">
            {/* 背景层 - 根据滚动状态显示不同样式 */}
            {isScrolled ? (
                // 滚动后：液态玻璃背景
                ...
                <div className="flex items-center justify-end gap-2">
                    ...
                    {userLoading ? (
                        ...
                        <>
                            <Link
                                href="/login"
                                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300 backdrop-blur-sm shadow-sm ${
                                    isScrolled
