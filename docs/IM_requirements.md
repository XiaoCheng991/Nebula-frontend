...
## 1. 目标概览
- 基于现有的 Spring Boot + JWT 认证系统，完善并落地以下六大能力：记忆身份、图片/文件上传、分离的单聊与群组、多端消息传输性能、@提及、以及头像设置，确保在用户体验、数据一致性、与后端安全方面均有明确约束与验收标准。
...
### 2) 图片和文件上传功能（高优先级）
- 描述：实现图片和文件的上传、预览和发送，使用 MinIO 对象存储。
- 需求细节：
...
### 4) 消息发送速度优化（中/高优先级）
- 描述：通过使用 WebSocket、缓存、以及必要时引入中间件（如 Redis）提升消息发送、订阅与渲染速度。
- 关注点：
  - 实时订阅的吞吐、延迟、丢包容忍性。
  - 客户端缓存策略和消息幂等性处理。
- 验收标准：
...
- 需求要点：
  - 支持上传头像到 MinIO，更新用户档案中的 avatar_url。
  - 提供默认头像选项，清晰回退处理。
- 验收标准：
  - 更新头像后在 UI 侧即时生效，消息气泡中的头像也应同步更新。

## 3. 数据模型简述（基于现有 PostgreSQL 数据库）
- 现状：JWT 认证体系，用户信息存储在 user_profiles 表。
- 计划的核心表：
...
## 4. 风险与前提
- 你需要确保 JWT 认证已正确配置，Token 与当前数据库结构一致。
- MinIO 存储桶需正确配置并设置适当的访问权限。
- 变更涉及多张表的结构，需注意迁移顺序，避免在上线阶段产生数据不一致。

## 5. 里程碑与交付
- 阶段 1：实现用户身份记忆与 JWT 驱动的进入流程
- 阶段 2：实现图片/文件上传（MinIO）
- 阶段 3：实现单聊/群组与对话切换
- 阶段 4：实现消息发送优化与 WebSocket 实时订阅
- 阶段 5：实现 @ 提及
- 阶段 6：实现头像设置
